...
////// TEMP for writing out story data to the new system////////


function writeStoryContent(pageInstances) {
    let saveFile = {}
    for( let i=0; i< pageInstances.length; i++) {
        let current = pageInstances[i]
        console.log(current)
    ...
    fs.writeFileSync("./keeper-of-the-labyrinth.json", JSON.stringify(saveFile)), 1000
}
...
function parseSequence(sequenceInstance, saveFile) {
    for (let page=0; page < sequenceInstance._pages.length; page++) {
        ...
        let destination;
        if (page == sequenceInstance._pages.length - 1) {
            destination = sequenceInstance._next?.title
        } else {
...
function parseStoryNode(nodeInstance, saveFile) {
    ...
    for (let option = 0; option < nodeInstance._buttons.length; option++) {
        let destination = nodeInstance._buttons[option][1].title 
